...
- Define the minimum authoritative actor, workforce-role, facility-scope, activation, revocation, and session persistence and migration boundary in the Sprint 15 implementation specification before runtime coding; do not infer it from token claims or practitioner-facility rosters.
...
- Implement only the [approved bounded Sprint 15 workforce authentication and authorization foundation](./ACCESS_CONTROL_BASELINE.md#recorded-approval), using synthetic data under its explicit production gate.
